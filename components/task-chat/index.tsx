...
export default function TaskChat({
  ...
  const loadTaskData = async () => {
    if (!taskId) {
      setLoading(false)
      setMessages([]) // Clear messages when no task ID
      return
    }

    setLoading(true)
    // Clear messages immediately when loading a new task
    setMessages([])
    ...
    try {
      ...
      if (task) {
        ...
        // Force empty messages initially
        setMessages([])

        // Add a small delay before loading messages to ensure any cached data is cleared
        setTimeout(async () => {
          try {
            // Load messages for this task
            const messagesData = await getMessagesForTask(taskId)
            // Double check that these are actually new messages
            if (messagesData && Array.isArray(messagesData)) {
              console.log(`Loaded ${messagesData.length} messages for task ${taskId}`)
              setMessages(messagesData)
            } else {
              console.log(`No messages found for task ${taskId}`)
              setMessages([])
            }
          } catch (error) {
  ...
  useEffect(() => {
    if (activeSubtask !== null) {
      const subtask = subtasks.find((s) => s.task_id === activeSubtask)
  ...
  const handleSubtaskClick = async (subtaskId: number) => {
    ...
    // Clear messages immediately when switching subtasks
    setMessages([])
    ...
    if (activeSubtask === subtaskId) {
      ...
      try {
        const messagesData = await getMessagesForTask(taskId, subtaskId)
        setMessages(messagesData || [])
      } catch (error) {
        console.error("Error loading messages:", error)
        setMessages([])
  ...
  const getActiveSubtask = () => {
    return subtasks.find((subtask) => subtask.task_id === activeSubtask)
  }

  // Update the handleSendMessage function to use Grok
  const handleSendMessage = async (text: string) => {
    if (!taskId || !text.trim() || !currentTask) return
    ...
    try {
      ...
      if (newMessage) {
        ...
        if (activeSubtask) {
          ...
          if (subtask) {
            activityText = `Message in subtask "${subtask.description}": ${text.substring(0, 50)}${text.length > 50 ? "..." : ""}`
          }
        }

        // Get subtask completion information
        const completedSubtasks = subtasks.filter((s) => s.status === "done").length
        const subtaskCount = subtasks.length

        // Create a task event message in the master chat
        await createMessage(
          activityText,
          "event",
          0, // Master chat task_id
          0, // Master chat subtask_id
          null, // System message
          {
            eventType: "task",
            title: `Task: ${currentTask.description}`,
            taskId: taskId,
            taskName: currentTask.description,
            subtaskId: activeSubtask || 0,
        ...
        if (onTaskActivity) {
          onTaskActivity(taskId, currentTask.description, activityText)
        }
  ...
  const markAsDone = async (subtaskId?: number) => {
    ...
    try {
      ...
      if (subtaskId) {
        ...
        const subtask = subtasks.find((s) => s.task_id === subtaskId)
        if (!subtask) {
        ...
        await updateTaskStatus(subtaskId, "done")
        ...
        const activityText = `Subtask "${subtask.description}" marked as done`

        // Get updated subtask completion information
        const updatedSubtasksData = await getSubtasks(taskId)
        const completedSubtasks = updatedSubtasksData.filter((s) => s.status === "done").length
        const subtaskCount = updatedSubtasksData.length
        ...
        // Notify the master chat about this activity
        console.log("Calling onTaskActivity for subtask:", taskId, currentTask.description, activityText)
        if (onTaskActivity) {
          onTaskActivity(taskId, currentTask.description, activityText)
        }
      } else {
        // Mark main task as done
        await updateTaskStatus(currentTask.task_id, "done")

        // Create activity text
        const activityText = `Task "${currentTask.description}" marked as done`
        ...
        await createMessage(
          ...
          null, // System message
          {
            ...
            taskName: currentTask.description,
            isNewSession: false,
            completedSubtasks: subtasks.length, // All subtasks are considered complete when the task is done
        ...
        // Notify the master chat about this activity
        console.log("Calling onTaskActivity for task:", taskId, currentTask.description, activityText)
        if (onTaskActivity) {
          onTaskActivity(taskId, currentTask.description, activityText)
        }

        // Move to next task
        const currentIndex = taskQueue.findIndex((t) => t.task_id === taskId)
        if (currentIndex < taskQueue.length - 1 && onSwitchTask) {
          onSwitchTask(taskQueue[currentIndex + 1].task_id)
        }
      }
    } catch (error) {
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div ref={taskHeaderRef}>
        ...
        {/* Task Queue */}
        {expanded && <TaskQueue tasks={taskQueue} activeTaskId={taskId} onSelectTask={onSwitchTask} />}
      </div>
      ...
      {activeSubtaskData && (
        <div
          ...
          onClick={() => handleSubtaskClick(activeSubtaskData.task_id)}
        ...
        >
          <div className="flex items-center">
            ...
            <span className={cn(activeSubtaskData.status === "done" && "text-gray-500")}>
              {activeSubtaskData.description}
            </span>
          </div>
          <ChevronUp size={18} />
        </div>
      ...
      <div ref={contentRef} className="flex-1 overflow-auto pb-48 pt-4 no-scrollbar">
        {activeSubtask === null ? (
          ...
          <>
                ...
                <div className="p-4">
                  <div className="mb-4 text-gray-700">
                    ...
                    <ul className="list-disc pl-5 mt-2 space-y-1">
                      <li>Research at least 5 direct competitors</li>
                      <li>Analyze their product offerings</li>
                      <li>Identify their pricing strategies</li>
                      <li>Note their unique selling propositions</li>
                    </ul>
                  </div>
            ...
            {hasSubtaskMessages && <MessageList messages={messages} />}
            {isLoading && (
              <div className="flex justify-center my-4">
                <div className="flex items-center space-x-2">
                  <div
